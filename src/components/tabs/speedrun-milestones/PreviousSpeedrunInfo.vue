  ...
  computed: {
    segmentAttr() {
      return {
        ...
        text: this.prevRunInfo.isSegmented ? "분할 스피드런" : "단일 구간 스피드런",
      };
    },
    stdAttr() {
      return {
        icon: `fas fa-coins o-icon ${this.prevRunInfo.usedSTD ? "l-icon-on" : "l-icon-off"}`,
        text: this.prevRunInfo.usedSTD ? "STD 업그레이드 사용" : "STD 업그레이드 미사용",
      };
    },
    offlineAttr() {
      ...
      return {
        ...
        text: `오프라인 시간 ${offlineFrac === 0 ? "사용 안 함" : formatPercents(offlineFrac, 1)}`,
      };
    },
    seedAttr() {
      let symbol;
...
<template>
  ...
  >
    ...
    <span
      ...
      :class="attr.icon"
    />
    <span>시작 시각: {{ startDate }}</span>
    <span>최종 기록: {{ finalTime }}</span>
  </div>
  <div
    v-else
    class="c-no-record"
