...
export default {
  ...
  computed: {
    ...
    setName() {
      return this.currentSet?.name ?? "선택 안 함";
    },
    setContents() {
      const contents = [];
      ...
      if (this.symbols) contents.push(quantify("기호", this.symbols.length, 0, 0, x => x));
      if (this.colors) contents.push(quantify("색상 조합", this.colors.length, 0, 0, x => x));
      return contents.join(" 및 ");
    },
    symbols() {
