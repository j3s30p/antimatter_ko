...
export class Sacrifice {
  ...
  static get disabledCondition() {
    if (NormalChallenge(10).isRunning) return "8차 차원 비활성화됨";
    if (EternityChallenge(3).isRunning) return "영원 도전 3 진행 중";
    if (DimBoost.purchasedBoosts < 5) return `차원 가속 ${formatInt(5)}회 필요`;
    if (AntimatterDimension(8).totalAmount.eq(0)) return "8차 반물질 차원 없음";
    if (this.nextBoost.lte(1)) return `${formatX(1)} 배율`;
    if (Player.isInAntimatterChallenge) return "도전 목표 도달";
    return "빅 크런치 필요";
  }
