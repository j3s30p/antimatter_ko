...
4. 압축을 푼 폴더에서 `resources` 폴더만 게임 폴더에 끌어다 놓습니다.
...
Steam 업데이트나 무결성 검사를 실행하면 한국어 패치가 제거될 수 있습니다. 이 경우 게임 버전과 패치 지원
버전이 같은지 확인한 뒤 패치를 다시 설치하세요.
...
- 원문이 정식 명칭이면 정식 명칭으로, 약어이면 약어로 표기합니다.
- 숫자, 공식, 단축키, 내부 식별자, 오토메이터 명령어와 저장 데이터 형식은 변경하지 않습니다.
- 버튼과 짧은 안내문은 화면 문맥에 맞게 간결하게, 도움말과 설명문은 자연스럽고 명확하게 번역합니다.
- 번역 후 실제 화면과 게임 진행에서 문맥, 줄바꿈, 기능 호환성을 검수합니다.
